...
pub fn can_add_and_retrieve_entries() {
    ...
    let trips = mk_trips();
    let mut ts: Series<BikeTrip> = emseries::Series::open("var/can_add_and_retrieve_entries.json")
...
pub fn can_search_for_an_entry_with_exact_time() {
    ...
    let trips = mk_trips();
    let mut ts: Series<BikeTrip> = emseries::Series::open(
...
pub fn can_get_entries_in_time_range() {
    ...
    let trips = mk_trips();
    let mut ts: Series<BikeTrip> = emseries::Series::open("var/can_get_entries_in_time_range.json")
        .expect("expect the time series to open correctly");
    ts.put(trips[0].clone()).expect("expect a successful put");
    ts.put(trips[1].clone()).expect("expect a successful put");
    ts.put(trips[2].clone()).expect("expect a successful put");
...
pub fn persists_and_reads_an_entry() {
    ...
    {
        let mut ts: Series<BikeTrip> = emseries::Series::open(
            "var/persists_and_reads_an_entry.json",
        ).expect("expect the time series to open correctly");
    ...
    {
        let ts: Series<BikeTrip> = emseries::Series::open("var/persists_and_reads_an_entry.json")
            .expect("expect the time series to open correctly");
        match ts.search_sorted(
            time_range(
                Utc.ymd(2011, 10, 31).and_hms(0, 0, 0),
...
pub fn can_write_to_existing_file() {
    ...
    {
        let mut ts: Series<BikeTrip> = emseries::Series::open(
            "var/can_write_to_existing_file.json",
        ).expect("expect the time series to open correctly");
    ...
    {
        let ts: Series<BikeTrip> = emseries::Series::open("var/can_write_to_existing_file.json")
            .expect("expect the time series to open correctly");
        match ts.search_sorted(
            time_range(
                Utc.ymd(2011, 10, 31).and_hms(0, 0, 0),
...
pub fn can_overwrite_existing_entry() {
    ...
    let mut ts: Series<BikeTrip> = emseries::Series::open("var/can_overwrite_existing_entry.json")
        .expect("expect the time series to open correctly");
...
pub fn record_overwrites_get_persisted() {
    ...
    {
        let mut ts: Series<BikeTrip> = emseries::Series::open(
            "var/record_overwrites_get_persisted.json",
        ).expect("expect the time series to open correctly");
    ...
    {
        let ts: Series<BikeTrip> = emseries::Series::open(
            "var/record_overwrites_get_persisted.json",
        ).expect("expect the time series to open correctly");
